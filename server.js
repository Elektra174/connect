/**
 * SERVER.JS - v21.26.3 (ULTIMATE PLATINUM MASTER - EVOLUTION)
 * ========================================================
 ...
 * 🛡️ VALIDATION: Strict Joi Schemas for all incoming data.
 */
...
const logger = winston.createLogger({
    ...
    transports: [
        ...
        new winston.transports.File({ filename: 'logs/training_data.log' }), // Для обучения будущих моделей
...
// --- 🛡️ ЗАЩИТА (RATE LIMITING & PROXY) ---
// Настройка trust proxy критична для работы лимитера за балансировщиком Render
...
const chatLimiter = rateLimit({
    ...
    legacyHeaders: false,
    message: { error: "Слишком много запросов. Система Connectum должна отдохнуть 15 минут." }
});
...
const CHANNEL_ID = '@psy_connectum'; 
const WEB_APP_URL = process.env.WEB_APP_URL;

const YANDEX_API_KEY = process.env.YANDEX_API_KEY;
const FOLDER_ID = process.env.YANDEX_FOLDER_ID;

// Инициализация Yandex Assistant через OpenAI SDK (REST Assistant API)
...
if (process.env.FIREBASE_SERVICE_ACCOUNT) {
    try {
        ...
        logger.error("Firebase Initialization Fail: " + e.message); 
    }
}
...
bot.onText(/\/start/, async (msg) => {
    ...
    const welcome = `
**Connectum | Platinum Evolution** 💫

Добро пожаловать в профессиональную ИИ-экосистему.

🧠 **Тренажер:** Отрабатывайте навыки на 30+ уникальных клиентах.
🤝 **Помощь:** Получите мгновенную ИИ-диагностику и поддержку.
...
Нажмите кнопку ниже, чтобы запустить приложение.
    ...
    try {
        ...
        logger.error("Start Command Error: " + e.message);
    }
});

async function adminLog(msg) {
    try { 
        await bot.sendMessage(ADMIN_ID, `📡 **Log v21.26.3**\n${msg}`, { parse_mode: 'Markdown' }); 
    } catch (e) {}
}

// --- 👥 ПОЛНАЯ БАЗА КЛИЕНТОВ (30 ДОСЬЕ - РАЗВЕРНУТО) ---
const CLIENT_DATABASE = {
    c1: { 
        id: "c1", name: "Виктория", age: 34, profession: "Маркетолог", gender: "female", 
        bio: "Парализующий саботаж при записи видео. Страх проявления зашкаливает. В теле — зажим в горле." 
    },
    c2: { 
        id: "c2", name: "Артем", age: 28, profession: "IT-разработчик", gender: "male", 
        bio: "Боюсь закончить масштабный заказ. Кажется, что результат будет бездарным. Тяжесть в плечах." 
    },
    c3: { 
        id: "c3", name: "Елена", age: 42, profession: "Бухгалтер", gender: "female", 
        bio: "Постоянное сжатие в груди и тревога. Не могу переключиться с работы на отдых." 
    },
    c4: { 
        id: "c4", name: "Михаил", age: 31, profession: "Фрилансер", gender: "male", 
        bio: "Сменил 5 профессий за 2 года. Нигде не находит признания, чувствует себя неудачником." 
    },
    c5: { 
        id: "c5", name: "Анна", age: 25, profession: "Студентка", gender: "female", 
        bio: "Не может завершить разрушительные отношения. Боится одиночества до тошноты." 
    },
    c6: { 
        id: "c6", name: "Игорь", age: 45, profession: "Топ-менеджер", gender: "male", 
        bio: "Достиг успеха, но внутри тотальная пустота. Онемение в животе и холод." 
    },
    c7: { 
        id: "c7", name: "Ольга", age: 38, profession: "Врач", gender: "female", 
        bio: "Ипохондрия. Паника при малейшем физическом дискомфорте." 
    },
    c8: { 
        id: "c8", name: "Дмитрий", age: 29, profession: "Продавец", gender: "male", 
        bio: "Боится встреч. Напряжение в скулах и зажим речи." 
    },
    c9: { 
        id: "c9", name: "Мария", age: 33, profession: "Домохозяйка", gender: "female", 
        bio: "Материнская вина. Ощущение, что она плохая мать. Не может вздохнуть." 
    },
    c10: { 
        id: "c10", name: "Сергей", age: 50, profession: "Предприниматель", gender: "male", 
        bio: "Банкротство бизнеса. Колоссальный стыд перед семьей." 
    },
    c11: { 
        id: "c11", name: "Юлия", age: 27, profession: "Модель", gender: "female", 
        bio: "РПП. Ненавидит свое отражение. Постоянный контроль веса." 
    },
    c12: { 
        id: "c12", name: "Андрей", age: 35, profession: "Архитектор", gender: "male", 
        bio: "Вспышки неконтролируемого гнева. Ощущение кипятка в груди." 
    },
    c13: { 
        id: "c13", name: "Наталья", age: 40, profession: "Учитель", gender: "female", 
        bio: "Одиночество в толпе. Живет как за толстым стеклом." 
    },
    c14: { 
        id: "c14", name: "Павел", age: 22, profession: "Курьер", gender: "male", 
        bio: "Зависимость от мнения родителей. Не может принять решение." 
    },
    c15: { 
        id: "c15", name: "Екатерина", age: 36, profession: "HR-директор", gender: "female", 
        bio: "Выгорание. Перфекционизм. Жжение в глазах от истощения." 
    },
    c16: { 
        id: "c16", name: "Александр", age: 44, profession: "Инженер", gender: "male", 
        bio: "Застрял в горе. Чувствует вину перед ушедшим близким." 
    },
    c17: { 
        id: "c17", name: "Светлана", age: 30, profession: "Бьюти-мастер", gender: "female", 
        bio: "Низкая самооценка. Считает себя 'недостаточной' для любви." 
    },
    c18: { 
        id: "c18", name: "Роман", age: 32, profession: "Аналитик", gender: "male", 
        bio: "Игровая зависимость. Уход от реальности в виртуальный мир." 
    },
    c19: { 
        id: "c19", name: "Ирина", age: 48, profession: "Юрист", gender: "female", 
        bio: "Синдром пустого гнезда. Смысл жизни пропал." 
    },
    c20: { 
        id: "c20", name: "Кирилл", age: 26, profession: "Дизайнер", gender: "male", 
        bio: "Агорафобия. Боится выходить на открытые пространства." 
    },
    c21: { 
        id: "c21", name: "Татьяна", age: 55, profession: "Пенсионерка", gender: "female", 
        bio: "Кризис старения. Ощущение, что время уходит впустую." 
    },
    c22: { 
        id: "c22", name: "Виктор", age: 39, profession: "Водитель", gender: "male", 
        bio: "Переживает измену. Колючая проволока вокруг сердца." 
    },
    c23: { 
        id: "c23", name: "Алина", age: 24, profession: "Бариста", gender: "female", 
        bio: "Не умеет говорить 'нет'. Чувствует, что все ею пользуются." 
    },
    c24: { 
        id: "c24", name: "Денис", age: 37, profession: "Охранник", gender: "male", 
        bio: "Навязчивые мысли о здоровье. Постоянные проверки." 
    },
    c25: { 
        id: "c25", name: "Людмила", age: 60, profession: "Педагог", gender: "female", 
        bio: "Конфликт с невесткой. Чувствует себя ненужной и лишней." 
    },
    c26: { 
        id: "c26", name: "Максим", age: 21, profession: "Блогер", gender: "male", 
        bio: "Подростковый бунт против системы. Ничего не хочет делать." 
    },
    c27: { 
        id: "c27", name: "Валерия", age: 31, profession: "Стилист", gender: "female", 
        bio: "Болезненная ревность. Постоянный поиск улик измены." 
    },
    c28: { 
        id: "c28", name: "Станислав", age: 43, profession: "Адвокат", gender: "male", 
        bio: "Трудоголизм. Не умеет расслабляться без алкоголя." 
    },
    c29: { 
        id: "c29", name: "Евгения", age: 29, profession: "Копирайтер", gender: "female", 
        bio: "Страх перемен. Боится менять работу, даже если там плохо." 
    },
    c30: { 
        id: "c30", name: "Константин", age: 35, profession: "Финансист", gender: "male", 
...
/**
 * ВЫЗОВ YANDEX GPT (REST ASSISTANT API)
 * Использует прямую интеграцию через responses.create
 */
 ...
 * Проверяет статус пользователя в канале @psy_connectum
 */
async function checkTelegramSub(userId) {
    if (userId === 'dev_platinum_master') return true;
    try {
        const member = await bot.getChatMember(CHANNEL_ID, userId);
        const activeStatuses = ['creator', 'administrator', 'member'];
        return activeStatuses.includes(member.status);
    } catch (e) { 
        logger.warn(`Subscription Check Failed for UID ${userId}: ${e.message}`);
        // В случае сбоя API — пускаем юзера, чтобы не блокировать сервис
        return true; 
    } 
}
 ...
 * Генерирует MP3 в формате base64
 */
async function generateYandexVoice(text, gender = 'female') {
    ...
    try {
        const res = await axios.post(url, new URLSearchParams({ 
            text, voice, folderId: FOLDER_ID, format: 'mp3', lang: 'ru-RU'
        }), {
        ...
        logger.error(`SpeechKit Synthesis Fail: ${e.message}`);
        return null; 
    }
}
 ...
 * Преобразует текст в вектор для поиска в Supabase
 */
...
async function getRAGContext(message, modalityId) {
    ...
    try {
        ...
        return docs ? docs.map(d => `ЭТАЛОННАЯ ИНТЕРВЕНЦИЯ: ${d.content}`).join('\n') : "";
    } catch (e) { 
        logger.warn("Supabase Vector Search Fail: " + e.message);
        return ""; 
    }
}
 ...
 * Использует транзакции Firestore для надежности
 */
async function spendDiamond(userId) {
    ...
    try {
        ...
        logger.error("Diamond Transaction Error: " + e.message);
        return true; 
    }
}
...
/**
 * СИНХРОНИЗАЦИЯ (v21.26.3): Подписка + Баланс + Фильтр клиентов
 */
app.get('/api/sync', async (req, res) => {
    const { userId } = req.query;
    if (!userId) return res.status(400).send("UID is mandatory");
    ...
    if (!db) {
        return res.json({ 
            isSubscribed: isSub, 
            diamonds: 5, 
            pool: Object.values(CLIENT_DATABASE).slice(0, 15) 
        });
    }
    ...
    try {
        ...
        res.json({
            ...
            pool: filteredPool.slice(0, 20)
        });
    } catch (e) { 
        logger.error("Sync API Error: " + e.message);
...
app.post('/api/chat', chatLimiter, async (req, res) => {
    ...
    try {
        ...
        // 1. Супервизор (Logging 2.0)
        if (action === 'get_hint') {
            const sys = PromptManager.generateSupervisorPrompt(modalityId, history, knowledge);
            const hint = await callYandexAi(`Психолог написал: ${message}. Дай короткий совет мастеру.`, sys, 0.3);
            
            if(db) await db.collection('training_logs').add({ 
                userId, type: 'supervisor_hint', context: message, hint, 
                timestamp: admin.firestore.FieldValue.serverTimestamp() 
            });
            return res.json({ hint });
        }

        // 2. Списание за старт новой сессии
        ...
        if(db) await db.collection('training_logs').add({ 
            userId, role, message, response: content, modalityId, 
            timestamp: admin.firestore.FieldValue.serverTimestamp() 
        });

        res.json({ content, voice });
    } catch (e) { 
        logger.error("Chat API Critical Fail: " + e.message);
        res.status(500).json({ error: "Ошибка обработки ИИ." }); 
    }
});
...
app.post('/api/finish', async (req, res) => {
    ...
    try {
        ...
        if (db && userId) {
            ...
            doc.text(`Method Score: ${analysis.method}%`, 50, 200);
            doc.moveDown();
            doc.fontSize(12).fillColor('#94a3b8').text(analysis.expert_comment || "Сессия завершена.", { width: 500 });
            doc.end();
        ...
        logger.error("Finish/Audit API Fail: " + e.message);
...
app.post('/api/profile', async (req, res) => {
    ...
    try {
        ...
        // Публичная витрина агрегатора ( Skill Rating выставляет ИИ)
...
app.listen(PORT, () => {
    logger.info(`🚀 Connectum v21.26.3 PLATINUM MASTER Online on port ${PORT}`);
    adminLog("🚀 Система v21.26.3 запущена на Yandex Assistant API.");
});
