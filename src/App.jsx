...
/**
 * =========================================================================
 * КОННЕКТУМ ПРО v25.0 - QUANTUM SINGULARITY
 * =========================================================================
 * * 📄 ФАЙЛ: src/App.jsx (ЕДИНЫЙ МОНОЛИТНЫЙ ФРОНТЕНД)
 * ⚖️ СТАТУС: ПРОИЗВОДСТВЕННЫЙ МАСТЕР-БИЛД
 * 🛡️ ПРОТОКОЛ: ANTI-CUTTING ACTIVE (1500+ СТРОК)
 * * 🏗️ АРХИТЕКТУРА:
 * 1. Система Премиум-Иконок (Custom SVG Engine)
 * 2. Глобальная База Кейсов (30 детальных сценариев МПТ)
 * 3. Ядро Квантового Дизайна (Advanced CSS & Animations)
 * 4. Компоненты Визуализации (Radar, Video, Charts)
 * 5. Управление Состояниями и Системная Инициализация
 * 6. Нейро-Интерфейс (Интегральный Чат)
 * 7. Командный Хаб и Клиентские Центры
 * 8. Система Роста и Геймификации (Quests)
 * 9. Элитная Витрина Мастеров (Marketplace)
 * 10. Терминал Оплаты и Управление Доступом
 * * 🎨 ДИЗАЙН-КОД: Google Stitch / High-End Cyber Studio
 * -------------------------------------------------------------------------
 */

// --- 1. СИСТЕМА ПРЕМИУМ-ИКОНОК (ВСТРОЕННЫЙ SVG ПАКЕТ) ---
// Каждая иконка отрисована вручную для идеального соответствия сетке 24x24
const Icons = {
  Infinity: ({ className }) => (
...
const CLIENT_DATABASE = [
    ...
    { id: "c3", name: "Елена", age: 42, profession: "Бухгалтер", status: "Средний", avatar: "👩‍💼", bio: "Постоянное сжатие в груди и тревога. Не может переключиться с работы на отдых. Ощущение, что за углом ждет катастрофа.", markers: ["СЖАТИЕ В ГРУДИ", "ТРЕМОР ПАЛЬЦЕВ", "ПОВЫШЕННЫЙ ПУЛЬС"] },
...
const GlobalStyles = () => (
  <style>{`
    ...
    @media (max-width: 480px) {
       ...
       .p-10 { padding: 1.5rem; }
       .rounded-[5rem] { border-radius: 3rem; }
    }
  `}</style>
);
...
const RadarChart = ({ data }) => {
    ...
    return (
      <div className="relative my-6 flex items-center justify-center">
        <svg width={size} height={size} className="overflow-visible radar-svg">
  ...
  if (screen === 'loading') return (
    <div className="screen-container bg-[#020617] items-center justify-between py-16 px-8 text-center">
      ...
      <div className="relative z-10 w-full max-w-xs mx-auto space-y-14">
        <div className="flex flex-col gap-5 text-left font-mono text-[9px] text-white/30 uppercase tracking-[0.2em] leading-relaxed border-l border-white/5 pl-6">
        ...
        <p className="text-white/20 text-[10px] font-bold tracking-widest uppercase italic">Синергия мастерства и доверия</p>
      </div>
    </div>
  );
      ...
      {!['loading', 'legal', 'chat'].includes(screen) && (
        <header className="sticky top-0 z-50 p-6">
          <div className="glass-panel rounded-[2rem] flex items-center justify-between p-4 border-white/15 shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden group">
            ...
            <button onClick={()=>{ triggerHaptic(); setScreen('store'); }} className="bg-primary/20 px-6 py-3 rounded-[1.2rem] border border-primary/40 flex items-center gap-4 active:scale-90 transition-all shadow-2xl group hover:border-primary">
          ...
          <div className="p-8 space-y-16 animate-in slide-in-from-right no-scrollbar pb-52">
            ...
            <div className="fixed bottom-0 left-0 right-0 p-12 z-50 pointer-events-none">
              <div className="max-w-xl mx-auto pointer-events-auto">
                <button onClick={() => { triggerHaptic('heavy'); setScreen('chat'); handleSend('Здравствуйте', true); }} className="premium-shine w-full bg-primary h-24 rounded-[4rem] flex items-center justify-between px-16 shadow-[0_0_100px_rgba(104,31,239,0.7)] border-t border-white/30 active:scale-[0.95] transition-all transform duration-500 group relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-8 relative z-10">
                    ...
                    <span className="text-3xl font-black tracking-tighter text-white uppercase italic">Начать сессию</span>
                  </div>
        ...
        {screen === 'store' && (
           <div className="p-10 space-y-16 animate-in zoom-in duration-700 pb-52 text-left">
              ...
              <div className="space-y-10">
                  {[
                    {id: 'test', title: 'Test-Drive', price: '490₽', desc: '5 сессий глубокой тренировки + полный методический аудит навыков от ИИ-Супервизора.', color: 'orange-500', gemsCount: '5', label: 'Entry Pass'},
                    {id: 'pro', title: 'PRO Terminal', price: '2990₽', desc: 'Безлимит на месяц + Золотые PDF сертификаты + Приоритетное размещение в Витрине Элиты.', color: 'primary', gemsCount: 'MAX', hot: true, label: 'Expansion Mode'},
                    {id: 'client', title: 'Pilot Platinum', price: '1990₽', desc: 'Безлимитный ИИ-терапевт + Квантовая Карта Осознанности + Доступ к закрытому клубу мастеров.', color: 'cyan-glow', gemsCount: 'ULTRA', label: 'Specialized Interface'}
                  ].map(t => (
